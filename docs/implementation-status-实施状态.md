...
- 本地 Run 恢复：Host 冷重启后不伪造重连只存在旧进程内存中的 `spawn/fork` Worker；非终态本地 Run 在租约内保留所有权，到期后幂等追加过期失败并将同 revision 节点恢复 `ready`。`pactflow_retry_node` 只允许无活动 Run 的 `failed/cancelled` 节点通过 revision CAS 恢复，旧 Run/claim/outcome/attempt 不覆盖，新派发创建 attempt+1。
