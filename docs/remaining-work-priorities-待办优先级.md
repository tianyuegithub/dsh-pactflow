...
| # | 事项 | 说明 |
| --- | --- | --- |
| 11 ◑ | 多宿主并发、真实依赖链矩阵 | **真实依赖链矩阵已完成**（suites 6/6 + 两节点 dogfood）；**跨进程锁互斥**已用真实 4 进程验证；**跨主机语义已合同化并验证**（change `harden-cross-host-lock-semantics`，已归档：异宿主遗留锁失败关闭且绝不夺取——真实子进程验证；超时错误指名持有 host/pid；同宿主死主恢复回归守卫）。**剩真实 NFS 双客户端互斥验证**：集群无 RWX、本机挂载需 sudo → **未运行**（精确 runbook 已入运维手册 §6.1 并由测试绑定，取得证据前不宣称已验证） |
