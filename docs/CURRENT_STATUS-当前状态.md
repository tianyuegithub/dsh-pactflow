...
**基线**：分支 `codex/pactflow-hardening`；`pnpm run check` = 68 文件 / 536 测试 / 13 包产物，全绿；`git diff --check` 通过；
`openspec validate --all --strict` 通过（**33 个 spec / 40 个已归档 change**）。**本批已提交到本地分支，尚未推送**（提交数随改动增长，故不在此写死）。
