# node-rerun-authorization Specification

## Purpose

让已成功的节点能在**所有者显式授权**下按新输入重做。此前 `succeeded` 是死路：上游产生新提交后，下游要么带着已经不对的输入继续显示为成功，要么被复制成一个语义相同的新节点，使历史与审批链就此分叉。

这是对架构 §3 终局能力 4「终态复活在事件折叠层被拒绝」的**有意削弱**，措辞已相应修订为「未经所有者显式授权的终态复活」。三条补偿把削弱限制在可控范围：授权只能来自人（排除一切自动路径，含挂机）、全部历史 Run 保留、下游只标记不级联——上游这次改动要不要紧，一次注释修改和一次接口签名变更在这里长得一模一样，只有人能判断。
