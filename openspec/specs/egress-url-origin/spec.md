# egress-url-origin Specification

## Purpose
约束全部出网目标的**来源**：Agent 面只能以注册 id 引用出网目标（Gitea Provider、K3s 模板、模型连接等），由 Host 从注册设置解析端点，工具 schema 不得暴露任何 URL/端点参数；操作员设置表单的探测辅助可携带草稿端点但凭据仅经注册引用解析。两者在未解析到注册配置时都必须在零出网下失败关闭——使「谁控制 URL」成为仓库内可验证的事实，而非依赖静态扫描器的推断。
