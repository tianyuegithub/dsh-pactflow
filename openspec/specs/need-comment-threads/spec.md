# need-comment-threads Specification

## Purpose

给需求级讨论一个持久载体。此前人与模型关于某个需求的往返只存在会话正文里，换一个会话就读不到、也无法按需求检索；审批理由是门禁产物而非讨论载体，只在四个评审点产生，承载不了评审点之间的分析、质疑与决定依据；模型的前期调研结论除非折进节点产物，否则随会话正文沉底。

本能力定义评论的对象归属与**由宿主强制判定**的作者身份、「评论永不构成任何门禁授权」、Agent 面的读写边界与计数预算、以**作废代替删除**（log-only 仅追加，历史不可篡改）、进入模型上下文时的来源与作者标记，以及事件词汇升版时的旧日志可读与老会话可写。
